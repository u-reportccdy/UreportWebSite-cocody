...
export function Footer() {
  ...
  const isSimplifiedPage = [
    PATHS.PUBLIC.PRIVACY,
    PATHS.PUBLIC.TERMS,
    PATHS.PUBLIC.COOKIES,
    PATHS.PUBLIC.LEGAL,
    PATHS.PUBLIC.REFUND,
    PATHS.PUBLIC.CONTRIBUTION_PAYMENT,
    PATHS.PUBLIC.MEMBER_PROFILE,
  ].some(path => location.pathname.startsWith(path)) || location.pathname.includes('/checkin');

  return (
    <footer className={`relative overflow-hidden bg-[#1e2a3f] text-white ${isSimplifiedPage ? 'pt-8 pb-24 lg:pb-24' : 'pt-20 pb-24 lg:pb-28'}`}>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`${!isSimplifiedPage ? 'border-t border-white/10 pt-8' : ''} flex flex-col gap-4 md:flex-row md:items-center md:justify-between`}>
